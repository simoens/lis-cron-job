...
def parse_table_from_soup(soup):
    ...
    for row in rows[1:]: 
        ...
        # DEBUG: Als nog steeds geen ID, dump de HTML van de rij
        if 'ReisId' not in bestelling:
             # Log alleen voor bekende MPET schepen
             naam = bestelling.get('Schip', '')
             if naam and ('MSC' in naam or 'WEC' in naam):
                  logging.warning(f"DEBUG ROW HTML ({naam}): {row}")
...
def haal_bestellingen_op(session):
    log_force("--- Running haal_bestellingen_op (v72, Verbose Trace) ---")
    try:
